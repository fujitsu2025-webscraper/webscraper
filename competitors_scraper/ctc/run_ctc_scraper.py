...
from ctc_scraper.utils.logger import setup_logger
from ctc_scraper.utils.webdriver_manager import initialize_webdriver
from ctc_scraper.utils.data_processor import save_to_csv, save_to_json, save_page_source
from ctc_scraper.models.scraper import CTCScraper
from ctc_scraper.config.settings import OUTPUT_BASE_DIR
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='CTCの事例ページをスクレイピングするスクリプト')
...
def main():
    ...
    try:
        # スクレイパーの初期化
        scraper = CTCScraper(driver, summarize=args.summarize)
        ...
        # ログファイルのパスを表示
        log_file = os.path.join(output_dir, "ctc_scraper_log.txt")
        print(f"\n実行ログは以下に保存されました: {log_file}")
        print(f"出力ファイルは {output_dir} ディレクトリに保存されました。")
        print("\n使用方法:")
        print("  基本実行: python run_ctc_scraper.py")
        print("  ヘッドレスモード: python run_ctc_scraper.py --headless")
        print("  要約機能: python run_ctc_scraper.py --no-summarize")
        print("\n")
