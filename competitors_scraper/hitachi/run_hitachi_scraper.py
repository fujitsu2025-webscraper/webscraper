...
from hitachi_scraper.models.scraper import Hitachi_Scraper
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='日立製作所の事例ページをスクレイピングするスクリプト')
...
def main():
    ...
    try:
        # スクレイパーの初期化
        scraper = Hitachi_Scraper(driver, max_clicks=args.max_clicks, summarize=args.summarize)
        ...
        # ログファイルのパスを表示
        log_file = os.path.join(output_dir, "hitachi_scraper_log.txt")
        print(f"\n実行ログは以下に保存されました: {log_file}")
        print(f"出力ファイルは {output_dir} ディレクトリに保存されました。")
        print("\n使用方法:")
        print("  基本実行: python run_hitachi_scraper.py")
        print("  クリック回数指定: python run_hitachi_scraper.py --max-clicks 10")
        print("  ヘッドレスモード: python run_hitachi_scraper.py --headless")
        print("  要約機能: python run_hitachi_scraper.py --no-summarize")
        print("\n")
