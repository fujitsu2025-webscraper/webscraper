...
import re
import logging
from ..config.settings import INDUSTRY_LIST, IBM_INDUSTRY_MAPPING, EXPLICIT_COMPANY_MAPPING, INDUSTRY_SYSTEM_MESSAGE
...
def determine_industry(url=None, company_name=None, business_type=None, business_field=None, customer_profile=None):
    ...
    if industry == "その他" and business_type:
        for ibm_industry, mapped_industry in IBM_INDUSTRY_MAPPING.items():
            if ibm_industry in business_type:
                industry = mapped_industry
                logger.info(f"IBMの業種分類に基づいて判断されたインダストリー: {industry}")
                break
